                ...
                docClient.get(params, function(err, data) {
                    if (err) {
                        ...
                        if (data.Item) {
                            ...
                            if (sessionAttributes.playSpellDetails === true) {
                                // delete sessionAttributes.playSpellDetails
                                // handlerInput.attributesManager.setSessionAttributes(sessionAttributes)
                                // speechText
                                // reprompt
                            } else {
                                // TODO: add tip, then logic to hear tip again once every 6 times
                                const spellDescriptionText = helper.getSpellDescriptionShort(data.Item)
                                let speechText = requestAttributes.t("SPELL_FOUND", {
                                    spellName: data.Item.name,
                                    lengthText: spellDescriptionText.lengthText,
                                    spellDescriptionShort: spellDescriptionText.text,
                                    footer: requestAttributes.t("SPELL_FOUND_ASK_FOR_DETAIL_PROMPT")
                                });
